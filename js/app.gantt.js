...
.directive('dhxGantt', function() {
  return {
    ...
    link:function ($scope, $element, $attrs, $controller){
     ...
     // ganttOriginal = Gantt.getGanttInstance();
     ...
     /* gantt.config.start_date="%D-%M-%d-%Y %H:%i";
      gantt.config.end_date="%D-%M-%d-%Y %H:%i";*/
...
.directive('dhxGanttOriginal', function() {
  return {
    restrict: 'A',
    scope: false,
    transclude: true,
    ...
    link:function ($scope, $element, $attrs, $controller){
      ...
      ganttOriginal = Gantt.getGanttInstance();
      $scope.$watch($attrs.data, function(collection){
        ganttOriginal.clearAll();
        ganttOriginal.parse(collection, "json");
      }, true);
      ...
      $scope.$watch(function() {
        ...
        ganttOriginal.setSizes();
      });

      //init gantt
      ganttOriginal.init($element[0]);
        ganttOriginal.config.autosize = "xy";
        ganttOriginal.attachEvent("onBeforeTaskDrag", function(id, mode, e){
              return false;
        });
