angular.module('app')
  /*.controller('holidayController', ['$location','$filter','$scope', '$state','$stateParams','$mdDialog','dialogFactory','JWTTOKEN', function($location,$filter,$scope,
    ...
    ////////////////////headingList/////////////////////////
    $scope.headingArray = [{"title":"From", "model": "blackout"},                    
                          {"title":"To"},
                          {"title":"Type"}];
    
    ///////////////////////////////////////////////////////
    // holidayFactory.getalholiday().then(function(holidayResult){
    //   JWTTOKEN.requestFunction('GET','blackouts?filter[include]=locationRelation&filter[where][deletedStatus]=false').then(function(blackoutResult){
         JWTTOKEN.requestFunction('GET','blackouts?filter[include]=machinecalendar').then(function(blackoutResult){
      /*console.log(blackoutResult.data.from);
      console.log(new Date(blackoutResult.data.from));
      blackoutResult.data.from=new Date(blackoutResult.data.from);
...
getMachines();
      ...
      var machineCalendar = $scope.selecteddata[0].machinecalendar;
      var machineCollection=[];
      for (var key in machineCalendar) {
        console.log(machineCalendar[key].machineID);
        console.log($filter('filter')(machines, {id:machineCalendar[key].machineID}));
      machineCollection.push($filter('filter')(machines, {id:machineCalendar[key].machineID})[0]);
      }
      console.log(machineCollection);
$scope.machineCollection=machineCollection;
        $mdDialog.show({
                parent: angular.element(document.body),
                targetEvent: event,
                clickOutsideToClose: true,
...
//            	Holiday.machinecalendar.destroyAll({id: $scope.selecteddata[0].id});
           ...
           /*     if ($scope.selecteddata[0].type ==  "BLACKOUT")
                  var selectedURL='machinecalendars/'+$scope.selecteddata[0].MachineID;
                else
                  ...
                  console.log(selectedURL);*/
                      ...
                      //JWTTOKEN.requestFunction('DELETE','holidays/'+$scope.selecteddata[0].id + '/machinecalendar').then(function(response){
                        JWTTOKEN.requestFunction('DELETE','blackouts/'+$scope.selecteddata[0].id+'/machinecalendar').then(function(response){
                       
                        console.log(response)
                    	  JWTTOKEN.requestFunction('DELETE','blackouts/'+$scope.selecteddata[0].id).then(function(res){
                    		       var ListArray=$scope.blackoutData;
                                var index = ListArray.indexOf($scope.selecteddata[0]);
                                ListArray.splice(index, 1);
