angular.module('app')
            ...
            JWTTOKEN.requestFunction('GET', 'mastertickets').then(function(result) {
                ...
                angular.forEach(result.data, function(value, key, obj) {
                   ...
                   // console.log(value.exFactoryDate);
                    ...
                    value.currentExFactoryDate=new Date(value.currentExFactoryDate).toDateString();
    ...
    function fitDataForDHTMLX_calendar(){
        ...
        $scope.showTabDialog = function(ev) {
                ...
                $mdDialog.show({
                        ...
                        controller: function DialogController($scope, $mdDialog, dataToPass) {
                            ...
                            //new changes done by rinsha for office color
                            /*$scope.office = [
                                { id: 1, name: 'Kannur', color: "purple" },
                                { id: 2, name: 'Mysore', color: "cyan"},
                                { id: 3, name: 'Noida', color: "teal"}
                              ];*/
                            ...
                            /* $scope.office = [
                               { id: 1, value: 'AX01',name:'Kannur' ,color: "yellow" },
                               { id: 2, value: 'AX04',name:'Mysore', color: "purple"},
                               { id: 3, value: 'AX03',name:'Noida', color: "blue"}
                             ];;*/
        ...
        $scope.addStatus = function(todayUpdate, valid) {
            ...
            updates.push({ "remark": todayUpdate.statusUpdates, "status": todayUpdate.status, "userID": userID, "date": new Date(), "eventDate": $scope.eventDate });
            todayUpdate.statusUpdates = updates;
            console.log(todayUpdate);

            ////new status updatesssss////////////////////

            if ($scope.machinecalendarData.statusUpdates != undefined) {
                var completeCountStatus = 0;
                for (k = 0; k < $scope.machinecalendarData.statusUpdates.length; k++) {
                    console.log($scope.machinecalendarData.statusUpdates[k].status);
                    if ($scope.machinecalendarData.statusUpdates[k].status == "completed") {
                        completeCountStatus++;
                    }
                    if (k == $scope.machinecalendarData.statusUpdates.length - 1) {
                        console.log(completeCountStatus);
                        console.log($scope.machinecalendarData.durationData)
                        if (parseInt(completeCountStatus) == parseInt($scope.machinecalendarData.durationData)) {
            ...
            JWTTOKEN.requestFunction('PUT', 'machinecalendars/' + $scope.machinecalendarData.id, { "statusUpdates": todayUpdate.statusUpdates, "status": todayUpdate.status, "lastStatusUpdateDate": todayUpdate.percentageofcompletion }).then(function(putdata) {
                console.log(putdata);
                $mdDialog.hide();
                $scope.todayUpdate = null;
                 getmachineEventsByDate(new Date($scope.machinecalendarData.date));

               // statusUpdatePercentageForAProcess($scope.machinecalendarData.processTicketID, todayUpdate.status);
            });
        }
